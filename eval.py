...
def get_args():
    ...
    parser.add_argument('--dataset', '-d', default='banCap', help='dataset')
...
def evaluate(args):
    ...
    word_map_file = os.path.join('banCap_5_caps_folder',
                                     'WORDMAP_banCap_5_cap_per_img_5_min_word_freq.json')
    with open(word_map_file, 'r') as j:
      word_map = json.load(j)
    rev_word_map = {v: k for k, v in word_map.items()}  # ix2word
    for hyp in hypotheses:
      words = ' '.join([rev_word_map[ind] for ind in hyp])
      print(words)
    ...
    with open('hypotheses.pkl', 'wb') as hf:
      pickle.dump(hypotheses, hf)
    
    with open('references.pkl', 'wb') as rf:
      pickle.dump(references, rf)
